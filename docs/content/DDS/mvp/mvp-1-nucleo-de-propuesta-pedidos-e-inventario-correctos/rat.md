...
Contraste técnico externo (no es KPI de Komatsu): la prevención de oversell bajo concurrencia exige actualización atómica condicional (`UPDATE … WHERE available >= qty` / locks), no check-then-act (patrones de motores de reserva; p. ej. decremento atómico + prueba concurrente con stock fijo). Umbrales del PoC vienen del profile (`0 oversell` en perímetro seed; p95/error en MVP 2–3). `evidencia` técnica externa · `hipótesis` de aplicabilidad al sitio Komatsu.
...
- Origen: FODA-D (AS-IS no observado) | profile problema | DT Empatizar plan
...
- ID: R2
- Supuesto: La implementación de `ReservarStockAlCrearPedido` (transacción + actualización condicional / control de concurrencia) logra **0 oversell** cuando N clientes concurrentes compiten por stock insuficiente **en el perímetro del PoC (seed)**.
...
- ID: R3
- Supuesto: El sponsor acepta la narrativa de **coexistencia** (PoC = capa de reserva demostrable) y **no** exige que MVP 1 reemplace NPS/WMS ni integre productivamente legacy.
- Etiqueta actual: `pendiente_campo`
- Impacto si falso: alto
- Incertidumbre: alta
- Falsación:
...
- Prioridad real distinta (pricing, multi-DC, EDI) desplaza el PoC — impacto alto × incertidumbre media; se vigila vía R1/R3.
- p95/error bajo carga cloud — diferido a MVP 2–3 (`evidencia` umbrales profile; no núcleo MVP 1).
- Verdad global de inventario Komatsu vía sync WMS — fuera de alcance MVP 1; no es RAT del núcleo seed.
