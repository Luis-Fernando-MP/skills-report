...
**Caso:** propuesta empresarial (PoC) a Komatsu — núcleo pedido↔stock con reserva (**0 oversell en perímetro del PoC**).  
...
| 4 | **Arquitecto / integración (lado Komatsu o partner)** | Evalúa si el PoC invade NPS/WMS o puede convivir como capa demostrable | `hipótesis` + ancla `evidencia` pública de existencia de portales/WMS |
| 5 | **Equipo entregable PoC (nosotros)** | Demuestra `ReservarStockAlCrearPedido`, 0 oversell (seed) y narrativa ejecutiva | `evidencia` (alcance profile / entregables MVP 1) |
...
**Prohibido en esta fase:** afirmar KPIs internos de Komatsu no aportados; afirmar que “no tienen sistema”; afirmar falla demostrada de Global Supplier Portal / KOM-MICS / ONEsLOGI / Swisslog SynQ.

### Evidencia pública (contexto industria — no AS-IS del sitio local)

| Hecho | Fuente (pública) | Etiqueta | Uso en la propuesta |
|-------|------------------|----------|---------------------|
| Komatsu impulsó **NPS (New Parts System)** / cadena de partes documentada en ecosistema dealer | Documentación pública NPS / dealer systems | `evidencia` | Posicionar el PoC como **capa demostrable** de reserva pedido↔stock, no como reemplazo unilateral |
| Ecosistema publicado incluye portales de proveedores, datos de planta (p. ej. KOM-MICS) y WMS regionales (p. ej. ONEsLOGI, Swisslog SynQ) | Fuentes citadas en `theme-audit-debate.md` | `evidencia` | El PoC **no** afirma sustituir WMS/ERP; la reserva del MVP 1 es lógica de dominio en **perímetro seed** |
| Aftermarket OEM construcción/minería prioriza disponibilidad de partes y uptime de máquina | Syncron Construction & Mining Aftermarket | `evidencia` proxy de dominio | Relevancia del problema de promesa de stock, sin KPIs Komatsu inventados |
| Oversell bajo concurrencia es un fallo clásico check-then-act; se mitiga con actualización atómica condicional / locks | Patrones de motores de reserva (p. ej. decremento atómico en PostgreSQL; literatura ingeniería) | `evidencia` técnica externa | Ancla de diseño de `ReservarStockAlCrearPedido` y de R2 |
...
**Insight de empatía (propuesta):** el valor percibido del MVP 1 no es “otro ERP”, sino **demostrar en vivo** (sobre seed) que un pedido solo existe si el stock se reservó de forma concurrente-segura, con rechazo limpio si no alcanza — y narrar cómo eso **podría** evaluarse junto a NPS/WMS existentes. `hipótesis` + ancla `evidencia` pública.
...
**Operadores de pedidos e inventario de partes en Komatsu** (y el sponsor que evalúa la propuesta) necesitan **crear pedidos que reserven stock de forma atómica y concurrente-segura dentro del perímetro del PoC** (y rechazar sin mutar cuando no hay disponibilidad), porque la propuesta empresarial debe demostrar **0 oversell** y una narrativa creíble de integración/escala **sin afirmar el reemplazo de NPS/WMS ni KPIs internos no aportados**.  
(`hipótesis` de dolor interno + `evidencia` de plataformas públicas + `evidencia` de criterio MVP 1 en profile)

### HMW

1. **PRIMARIO —** ¿Cómo podríamos **reservar stock en el mismo acto de crear el pedido** para que el **operador de pedidos** obtenga **aceptación con stock comprometido o rechazo HTTP sin mutación**, con **0 oversell bajo concurrencia** (seed/demo)?  
...
**ReservarStockAlCrearPedido + demo HTTP:** al `POST /pedidos`, el sistema valida líneas, **reserva stock** de forma atómica; si alguna línea no alcanza → **rechazo sin mutar** pedido ni inventario; bajo carga concurrente tipificada sobre seed → **0 oversell**. Catálogo/clientes/usuarios + seed de partes; ADR de arquitectura objetivo (microservicios + AWS); resumen 1–2 págs. para Komatsu.
...
**No es** despliegue AWS ni k6 sistemático (MVP 2–3). Auth PoC: JWT o API key (`evidencia` profile); Cognito/SSO fuera.
...
**Invariante (perímetro PoC):** `disponible = onHand - reservado` (o equivalente); nunca `reservado > onHand`. No se afirma verdad global de inventario Komatsu. `hipótesis` de modelo; validación numérica en Test/RAT.

### Flujo conceptual `ReservarStockAlCrearPedido`

1. Recibe líneas `(partNumber, cantidad)`.  
2. Abre unidad de trabajo (transacción).  
3. Por cada línea: lee stock con control de concurrencia (bloqueo pesimista o versión optimista / `UPDATE … WHERE disponible >= qty`).  
...
| `KT-SEAL-014` | 1 | 0 | Concurrencia / oversell |
...
- Excluye: ERP/MRP/EDI, multi-almacén global, IoT, reemplazo NPS/WMS, integración productiva legacy, EKS bloqueante, SSO enterprise, KPIs internos no aportados.
